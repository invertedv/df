...
func TestSQLdf_Where(t *testing.T) {
	...
	//	assert.Nil(t, e)
	_, e = df2.ParseExpr("l2 := if(prop_zip3=='005', 'yes','no')", df)
	assert.Nil(t, e)
	_, e = df2.ParseExpr("lexp := exp(latitude/100)", df)
	assert.Nil(t, e)
	_, e = df2.ParseExpr("test :='20221231'", df)
	assert.Nil(t, e)
	_, e = df2.ParseExpr("test1 :=int(4)", df)
	assert.Nil(t, e)
	_, e = df2.ParseExpr("test2 :=(test != '20221231' || latitude < 41) && test1 >= 3", df)
	assert.Nil(t, e)
	_, e = df2.ParseExpr("test3 :=test == '20221231' || latitude < 40 && test1 >= 3", df)
	assert.Nil(t, e)
	_, e = df2.ParseExpr("logical :=prop_zip3 == '005'", df)
	assert.Nil(t, e)
